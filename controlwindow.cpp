    ...
    ui(new Ui::ControlWindow)
{
    ...
    inputs[NHexonInit] = new InputWidget(this,200,150,0,999,200,"Starting Hexons");
    inputs[NObstacleInit] = new InputWidget(this,200,200,0,999,50,"Starting Obstacles");
    inputs[NFoodInit] = new InputWidget(this,200,250,0,999,25,"Starting Food");
}
...
void ControlWindow::on_startButton_clicked()
{
    ...
    for (int i = 0; i < inputs[NFoodInit]->value(); i++) // #from control window value
    {
        bool s = false;
        while (!s)
        {
            ...
            cube h = {a,b,-a-b};
            // add obstacle to map
            s = hex_map.AddHexItem(new Food(h), &scene);
        }
    }
